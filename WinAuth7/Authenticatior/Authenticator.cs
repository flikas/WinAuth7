...
namespace WindowsPhoneAuthenticator
{
	...
	public class Authenticator : ICloneable
	{
#if AutenticatorData
		[DataContract]
		private class AutenticatorData
		{
			[DataMember(Name = "servertimediff")]
			public string ServerTimeDiff;
			[DataMember(Name = "secretdata")]
			public string SecretData;
			[DataMember(Name = "restorecodeverified")]
			public string RestoreCodeVerified;


			public static AutenticatorData getCareerFromJSonStream(Stream stream)
			{
				DataContractJsonSerializer jsSerializer = new DataContractJsonSerializer(typeof(AutenticatorData));
				AutenticatorData aud = (AutenticatorData)jsSerializer.ReadObject(stream);
				return aud;
			}

			public static void setCareerToJSonStream(AutenticatorData aud, Stream stream)
			{
				DataContractJsonSerializer jsSerializer = new DataContractJsonSerializer(typeof(AutenticatorData));
				jsSerializer.WriteObject(stream, aud);
			}
		...
		/// Create a new Authenticator object
		/// </summary>
		public Authenticator(String description)
		{
			LoadedFormat = FileFormat.WinAuth;
		...
		/// <summary>
		/// Get Authenticator instance from Json stream
		/// </summary>
		public static Authenticator GetFromJSonStream(Stream stream)
		{
			DataContractJsonSerializer jsSerializer = new DataContractJsonSerializer(typeof(Authenticator));
			Authenticator au = (Authenticator)jsSerializer.ReadObject(stream);
			return au;
		}
		...
		public async Task Load(Stream stream, FileFormat format, string password)
		{
			// use our own format
			if (format == FileFormat.WinAuth)
			{
				...
				{
				   await LoadXmlSettings(xr, password, DEAFULT_CONFIG_VERSION);
					return;
				}
			}
		...
		/// <param name="password">optional encryption password</param>
		private async Task LoadXmlSettings(XmlReader xr, string password, decimal version)
		{
		...
		public void WriteXmlString(XmlWriter writer)
		{
			...
			if ((PasswordType & PasswordTypes.User) != 0)
			{
				...
				byte[] cipher = ProtectedData.Protect(plain, null, 0);
			...
			if ((PasswordType & PasswordTypes.Machine) != 0)
			{
				...
				byte[] plain = Authenticator.StringToByteArray(data);
				byte[] cipher = ProtectedData.Protect(plain, null, 1);
				data = Authenticator.ByteArrayToString(cipher);
